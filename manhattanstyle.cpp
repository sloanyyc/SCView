...
//        lineeditImage = QImage(":/core/images/inputfield.png"); // LOLO
        lineeditImage = QImage(":WhiteRect");
...
ManhattanStyle::ManhattanStyle(const QString &baseStyleName)
    : QCommonStyle(), d(new ManhattanStylePrivate(baseStyleName))
{
}
...
{
    QColor color = ManhattanStyle::panelTextColor();
...
void ManhattanStyle::polish(QWidget *widget)
{
    ...
    if (panelWidget(widget)) {
        if (qobject_cast<QToolButton*>(widget)) {
            ...
            widget->setMinimumHeight(navigationWidgetHeight());
                                       ...
                                       const QWidget *widget) const
{
    ...
    case QStyle::SP_ToolBarHorizontalExtensionButton: {
            static const QPixmap extButton(":extension");
            pixmap = extButton;
        }
        break;
    case QStyle::SP_TitleBarCloseButton: {
//			static const QPixmap closeButton(":/core/images/closebutton.png");  // original of Qt
            static const QPixmap closeButton(":closetab");						// LOLO
            pixmap = closeButton;
        }
        break;
    default:
                                   ...
                                   QPainter *painter, const QWidget *widget) const
{
    ...
    case PE_PanelStatusBar:
        {
            ...
            painter->setPen(borderColor().darker(110));
    ...
    case PE_IndicatorToolBarSeparator:
        {
            QColor separatorColor = borderColor();
    ...
    case PE_IndicatorToolBarHandle:
        {
            ...
            QColor dark = borderColor();
            dark.setAlphaF(0.4);

            QColor light = baseColor();
            light.setAlphaF(0.4);
                                 ...
                                 QPainter *painter, const QWidget *widget) const
{
    ...
    case CE_MenuBarItem:
        ...
        if (const QStyleOptionMenuItem *mbi = qstyleoption_cast<const QStyleOptionMenuItem *>(option)) {
            ...
            if (act) {
                // Fill|
                QColor cbaseColor = baseColor();
                QLinearGradient grad(option->rect.topLeft(), option->rect.bottomLeft());
                grad.setColorAt(0, cbaseColor.lighter(120));
                grad.setColorAt(1, cbaseColor.lighter(130));
    ...
    case CE_ComboBoxLabel:
        if (const QStyleOptionComboBox *cb = qstyleoption_cast<const QStyleOptionComboBox *>(option)) {
            if (panelWidget(widget)) {
                ...
                customPal.setBrush(QPalette::All, QPalette::ButtonText, panelTextColor());
    ...
    case CE_ToolBar:
        {
            QString key;
            key.sprintf("mh_toolbar %d %d %d", option->rect.width(), option->rect.height(), baseColor().rgb());;

            QPixmap pixmap;
            QPainter *p = painter;
            QRect rect = option->rect;
            if (usePixmapCache() && !QPixmapCache::find(key, pixmap)) {
            ...
            if (horizontal)
                horizontalGradient(p, gradientSpan, rect);
            else
                verticalGradient(p, gradientSpan, rect);
    ...
    case CC_ToolButton:
        if (const QStyleOptionToolButton *toolbutton = qstyleoption_cast<const QStyleOptionToolButton *>(option)) {
            ...
            if (toolbutton->state & State_HasFocus) {
                ...
                QColor focusColor = panelTextColor();
...
QColor ManhattanStyle::shadowColor()
{
...
QColor ManhattanStyle::highlightColor()
{
    QColor result = baseColor();
    result.setHsv(result.hue(),
...
QColor ManhattanStyle::borderColor()
{
    QColor result = baseColor();
    result.setHsv(result.hue(),
                  result.saturation(),
...
void ManhattanStyle::menuGradient(QPainter *painter, const QRect &spanRect, const QRect &clipRect)
{
    QString key;
    ...
    QColor menuColor = mergedColors(baseColor(), QColor(240, 240, 240), 25);
...
void ManhattanStyle::verticalGradient(QPainter *painter, const QRect &spanRect, const QRect &clipRect)
{
    QString key;
    key.sprintf("mh_toolbar %d %d %d %d %d", spanRect.width(), spanRect.height(), clipRect.width(),
                                             clipRect.height(), baseColor().rgb());;
...
void ManhattanStyle::horizontalGradient(QPainter *painter, const QRect &spanRect, const QRect &clipRect)
{
    QString key;
    key.sprintf("mh_toolbar %d %d %d %d %d", spanRect.width(), spanRect.height(),
                clipRect.width(), clipRect.height(), baseColor().rgb());
    QPixmap pixmap;
    QPainter *p = painter;
    QRect rect = clipRect;
    if (usePixmapCache() && !QPixmapCache::find(key, pixmap)) {
